...
function setupEbayPriceCalculator() {
  ...
  const roundedPriceResult = document.querySelector('#rounded-price-result');

  if (!purchasePriceInput || !weightInput || !categoryInput || !marginInput || !calculateButton || !purchaseResult || !shippingResult || !baseResult || !marginResult || !categoryResult || !feeResult || !priceResult || !roundedPriceResult) {
    return;
  }
  ...
  calculateButton.addEventListener('click', () => {
    ...
    if (!shippingCost) {
      ...
      priceResult.textContent = 'Sugerowana cena sprzedaży eBay: —';
      roundedPriceResult.textContent = 'Cena do wystawienia: —';
      return;
    }

    if (!purchasePrice || purchasePrice <= 0) {
      shippingResult.textContent = `Koszt wysyłki: ${shippingCost.toFixed(2)} €`;
      priceResult.textContent = 'Sugerowana cena sprzedaży eBay: wpisz cenę zakupu';
      roundedPriceResult.textContent = 'Cena do wystawienia: —';
      return;
    }

    if (margin < 0) {
      priceResult.textContent = 'Sugerowana cena sprzedaży eBay: marża nie może być ujemna';
      roundedPriceResult.textContent = 'Cena do wystawienia: —';
      return;
    }
    ...
    const roundedFinalPrice = getRoundedEbayPrice(finalPrice);
